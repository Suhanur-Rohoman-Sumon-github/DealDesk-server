...
import {  GraphPoint, TUser } from './user.interface';
import { JabedaModel, userModel } from './user.model';
...
import { CategoryModel } from '../product_category/product_category.model';
...
const getMyChanelFromDb = async (email:string) => {
  const user = await userModel.findOne({ email }, { myChanel: 1, username: 1, isAccountAproved: 1 });
 
  return {
    success: true,
    myChanel: user!.myChanel,
    username: user!.username,
    isPending: user!.isAccountAproved === "pending",
  };
};
...
const getUserOrderInsightsFromDb = async (userId: string) => {
  ...
  //   graphData.push({ date: dateStr, amount: dailyTotal, orders: dailyOrders.length });
  // }
